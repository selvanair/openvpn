...
/* helper to add an entry to the internal ipv4 routing table */
static void
multi_add_iroute_entry(struct multi_context *m, struct multi_instance *mi,
                       const struct iroute *ir)
{
    struct gc_arena gc = gc_new();
    if (ir->netbits >= 0)
...
/* helper to add an entry to the internal ipv6 routing table */
static void
multi_add_iroute6_entry(struct multi_context *m, struct multi_instance *mi,
                       const struct iroute_ipv6 *ir6)
{
    struct gc_arena gc = gc_new();
    msg(D_MULTI_LOW, "MULTI: internal route %s/%d -> %s",
                  ...
                  struct multi_instance *mi)
{
    ...
    if (TUNNEL_TYPE(mi->context.c1.tuntap) == DEV_TYPE_TUN)
    {
        ...
        for (ir = mi->context.options.iroutes; ir != NULL; ir = ir->next)
        {
            multi_add_iroute_entry(m, mi, ir);
        }
        for (ir6 = mi->context.options.iroutes_ipv6; ir6 != NULL; ir6 = ir6->next)
        {
            multi_add_iroute6_entry(m, mi, ir6);
        }
    }
}
